...
prompts as personal shortcuts within LS CLI. You can create commands that
...
LS CLI discovers commands from two locations, loaded in a specific order:
...
When a custom command attempts to execute a shell command, LS CLI will now
...
LS CLI will then execute the multi-line prompt defined in your TOML file.
