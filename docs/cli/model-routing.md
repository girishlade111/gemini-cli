...
LS CLI includes a model routing feature that automatically switches to a
...
LS CLI supports using a local model for routing decisions. When configured,
LS CLI will use a locally-running **Gemma** model to make routing decisions
...
The model used by LS CLI is determined by the following order of precedence:
