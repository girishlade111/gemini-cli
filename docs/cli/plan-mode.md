...
You can either configure LS CLI to start in Plan Mode by default or enter
Plan Mode manually during a session.

### Launch in Plan Mode

To start LS CLI directly in Plan Mode by default:

1.  Use the `/settings` command.
2.  Set **Default Approval Mode** to `Plan`.

To launch LS CLI in Plan Mode once:

1. Use `gemini --approval-mode=plan` when launching LS CLI.

### Enter Plan Mode manually

To start Plan Mode while using LS CLI:

- **Keyboard shortcut:** Press `Shift+Tab` to cycle through approval modes
  (`Default` -> `Auto-Edit` -> `Plan`). Plan Mode is automatically removed from
  the rotation when LS CLI is actively processing or showing confirmation
  dialogs.
...
- **Natural Language:** Ask LS CLI to "start a plan for...". LS CLI
  calls the
  [`enter_plan_mode`](../tools/planning.md#1-enter_plan_mode-enterplanmode) tool
  to switch modes. This tool is not available when LS CLI is in
  [YOLO mode](../reference/configuration.md#command-line-arguments).

## How to use Plan Mode

Plan Mode lets you collaborate with LS CLI to design a solution before
LS CLI takes action.

1.  **Provide a goal:** Start by describing what you want to achieve. LS CLI
    will then enter Plan Mode (if it's not already) to research the task.
2.  **Discuss and agree on strategy:** As LS CLI analyzes your codebase, it
    ...
    options using [`ask_user`](../tools/ask-user.md). **LS CLI will stop and
...
3.  **Review the plan:** Once you've agreed on the strategy, LS CLI creates
...
4.  **Approve or iterate:** LS CLI will present the finalized plan for your
    formal approval.
...
You can collaborate with LS CLI by making direct changes or leaving comments
in the implementation plan. This is often faster and more precise than
describing complex changes in natural language.

1.  **Open the plan:** Press `Ctrl+X` when LS CLI presents a plan for
    review.
...
4.  **Review and refine:** LS CLI automatically detects the changes, reviews
...
- **Approve a plan:** When LS CLI presents a finalized plan, approving it
...
- **Natural language:** Ask LS CLI to "exit plan mode" or "stop planning."
...
workflows. You can customize how LS CLI plans by using skills, adjusting
safety policies, changing where plans are stored, or adding hooks.

### Custom planning with skills

You can use [Agent Skills](../cli/skills.md) to customize how LS CLI
...
- A **"Security Audit"** skill could prompt LS CLI to look for specific
  vulnerabilities during codebase exploration.
- A **"Frontend Design"** skill could guide LS CLI to use specific UI
  components and accessibility standards in its proposal.

To use a skill in Plan Mode, you can explicitly ask LS CLI to "use the
`<skill-name>` skill to plan..." or LS CLI may autonomously activate it
...
Tell LS CLI it can use these tools in your prompt, for example: _"You can
...
the planning workflow or enforce additional checks when LS CLI transitions
...
Storage whenever LS CLI exits Plan Mode to start the implementation.
...
how LS CLI researches and stores plans for your specific domain.
...
When using an [auto model](../reference/configuration.md#model), LS CLI
...
LS CLI automatically and silently falls back to a faster model to ensure
...
By default, LS CLI automatically cleans up old session data, including all
...
When running LS CLI in non-interactive environments (such as headless
...
- **Automated implementation:** When exiting Plan Mode to execute the plan,
  LS CLI automatically switches to
...
[`plan.toml`]:
  https://github.com/google-gemini/ls-cli/blob/main/packages/core/src/policy/policies/plan.toml
[Conductor]: https://github.com/ls-cli-extensions/conductor
