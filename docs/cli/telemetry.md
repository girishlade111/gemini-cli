...
LS CLI provides built-in support for OpenTelemetry, transforming every agent
...
enterprise-wide usage in the cloud, LS CLI's observability system provides
the actionable intelligence needed to move from "black box" AI to predictable,
high-performance systems.

## OpenTelemetry integration

LS CLI integrates with **[OpenTelemetry]**, a vendor-neutral,
...
2.  Authenticate with Google Cloud using one of these methods:
    ...
    * **Method B: CLI Auth** (Direct export only): Simplest method for local
      users. LS CLI uses the same OAuth credentials you used for login. To
...
After you enable telemetry and run LS CLI, you can view your data in the
...
LS CLI provides a pre-configured
...
![LS CLI Monitoring Dashboard Overview](/docs/assets/monitoring-dashboard-overview.png)

![LS CLI Monitoring Dashboard Metrics](/docs/assets/monitoring-dashboard-metrics.png)

![LS CLI Monitoring Dashboard Logs](/docs/assets/monitoring-dashboard-logs.png)

To learn more, see
[Instant insights: LS CLI’s pre-configured monitoring dashboards](https://cloud.google.com/blog/topics/developers-practitioners/instant-insights-ls-clis-new-pre-configured-monitoring-dashboards/).
...
LS CLI includes identifiers in its `User-Agent` header to help you
...
LS CLI includes `session.id`, `installation.id`, `active_approval_mode`, and
`user.email` (when authenticated) as common attributes on all data.

### Logs

Logs provide timestamped records of specific events. LS CLI logs events
...
These logs track how LS CLI selects and routes requests to models.
...
LS CLI exports several custom metrics.
...
LS CLI provides detailed performance metrics for advanced monitoring.
...
- `gen_ai.operation.name`: High-level operation (for example, `tool_call`,
  ...
  `schedule_tool_calls`).
- `gen_ai.agent.name`: Set to `ls-cli`.
