...
LS CLI supports a variety of themes to customize its color scheme and
appearance. You can change the theme to suit your preferences via the `/theme`
command or `"theme":` configuration setting.

## Available themes

LS CLI comes with a selection of pre-defined themes, which you can list
using the `/theme` command within LS CLI:
...
Selected themes are saved in LS CLI's
...
LS CLI lets you create your own custom color themes by specifying them in
...
<!-- prettier-ignore -->
> [!WARNING]
> For your safety, LS CLI will only load theme files that
...
- Select your custom theme using the `/theme` command in LS CLI. Your custom
