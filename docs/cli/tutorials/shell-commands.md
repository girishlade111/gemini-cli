...
LS CLI also records the command and its output in the current session
...
You can automate tasks using a combination of LS CLI and shell commands.
...
LS CLI attempts to handle interactive commands (like `git add -p` or
...
Giving an AI access to your shell is powerful but risky. LS CLI includes
