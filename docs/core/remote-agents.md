...
LS CLI supports connecting to remote subagents using the Agent-to-Agent
(A2A) protocol. This allows LS CLI to interact with other agents, expanding
its capabilities by delegating tasks to remote services.

LS CLI can connect to any compliant A2A agent. You can find samples of A2A
...
LS CLI routes traffic to remote agents through an HTTP/HTTPS proxy if one is
...
Many remote agents require authentication. LS CLI supports several
...
LS CLI supports the following authentication types:
...
use, LS CLI opens your browser for sign-in and persists the resulting tokens
...
When LS CLI loads a remote agent, it validates your auth configuration
...
When connecting to a remote agent, LS CLI first fetches the agent card
...
Users can manage subagents using the following commands within LS CLI:
...
<!-- prettier-ignore -->
> [!TIP]
> You can use the `@cli_help` agent within LS CLI for assistance
