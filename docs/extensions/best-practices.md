# LS CLI extension best practices
...
Developing extensions for LS CLI is a lightweight, iterative process. Use
...
- **Restart the CLI:** Extensions are loaded at the start of a session. Restart
  LS CLI after making changes to the manifest or linking a new extension.
...
- **Test the command:** Run the server's `command` and `args` directly in your
  terminal to ensure it starts correctly outside of LS CLI.
