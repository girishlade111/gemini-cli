...
# LS CLI authentication setup

To use LS CLI, you'll need to authenticate with Google. This guide helps you
...
For most users, we recommend starting LS CLI and logging in with your
...
2. Select **Sign in with Google**. LS CLI opens a sign in prompt using your
...
To authenticate and use LS CLI with a Gemini API key:
...
To use LS CLI with Google Cloud's Vertex AI platform, choose from the
...
Cloud project for LS CLI to use. This applies when you meet at least one of
...
To configure LS CLI to use a Google Cloud project, do the following:
...
4.  Configure your environment variables. Set either the `GOOGLE_CLOUD_PROJECT`
    ...
    CLI. LS CLI checks for `GOOGLE_CLOUD_PROJECT` first, then falls back to
...
2.  **Use a `.env` file:** Create a `.gemini/.env` file in your project
    directory or home directory. LS CLI automatically loads variables from
...
When running LS CLI within certain Google Cloud environments, authentication
is automatic.

In a Google Cloud Shell environment, LS CLI typically authenticates
automatically using your Cloud Shell credentials. In Compute Engine
environments, LS CLI automatically uses Application Default Credentials
...
- [LS CLI: Quotas and Pricing](../resources/quota-and-pricing.md).
- [LS CLI: Terms of Service and Privacy Notice](../resources/tos-privacy.md).
