...
Hooks are scripts or programs that LS CLI executes at specific points in the
...
LS CLI waits for all matching hooks to complete before continuing.
...
Hooks are triggered by specific events in LS CLI's lifecycle.
...
3. **Debug via Stderr**: Use `stderr` for **all** logging and debugging (for
   example, `echo "debug" >&2`). LS CLI captures `stderr` but never attempts
   to parse it as JSON.

#### Exit codes

LS CLI uses exit codes to determine the high-level outcome of a hook
execution:
...
Hooks are configured in `settings.json`. LS CLI merges configurations from
...
3.  **System settings**: `/etc/ls-cli/settings.json`.
...
LS CLI **fingerprints** project hooks. If a hook's name or command changes
