# LS CLI companion plugin: Interface specification

> Last Updated: September 15, 2025

This document defines the contract for building a companion plugin to enable
LS CLI's IDE mode. For VS Code, these features (native diffing, context
awareness) are provided by the official extension
([marketplace](https://marketplace.visualstudio.com/items?itemName=Google.ls-cli-vscode-ide-companion)).
...
LS CLI and the IDE plugin communicate through a local communication channel.
...
For LS CLI to connect, it needs to discover which IDE instance it's running
