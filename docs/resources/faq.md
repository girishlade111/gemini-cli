...
problems encountered while using LS CLI.

## General issues

This section addresses common questions about LS CLI usage, security, and
troubleshooting general errors.

### Why can't I use third-party software like Claude Code, OpenClaw, or OpenCode with LS CLI?
...
your total token usage using the `/stats` command in LS CLI.

## Installation and updates

### How do I check which version of LS CLI I'm currently running?

You can check your current LS CLI version using one of these methods:

- Run `gemini --version` or `gemini -v` from your terminal
- Check the globally installed version using your package manager:
  - npm: `npm list -g @google/ls-cli`
  - pnpm: `pnpm list -g @google/ls-cli`
  - yarn: `yarn global list @google/ls-cli`
  - bun: `bun pm ls -g @google/ls-cli`
  - homebrew: `brew list --versions ls-cli`
- Inside an active LS CLI session, use the `/about` command

### How do I update LS CLI to the latest version?

If you installed it globally via `npm`, update it using the command
`npm install -g @google/ls-cli@latest`. If you compiled it from source, pull
...
- **Use a `.env` file:** Create a `.env` file in your project's `.gemini`
  directory (`.gemini/.env`) and store your keys there. LS CLI will
...
### Where are LS CLI configuration and settings files stored?

LS CLI configuration is stored in two `settings.json` files:

1.  In your home directory: `~/.gemini/settings.json`.
2.  In your project's root directory: `./.gemini/settings.json`.

Refer to [LS CLI Configuration](../reference/configuration.md) for more
details.
...
limits to Gemini Code Assist and LS CLI. These are shared across LS CLI
...
### What is the privacy policy for using Gemini Code Assist or LS CLI if I've subscribed to Google AI Pro or Ultra?
...
across both Gemini 2.5 Pro and Flash. They are shared quota across LS CLI
and agent mode in Gemini Code Assist IDE extensions. You can learn more about
quota limits for LS CLI, Gemini Code Assist and agent mode in Gemini Code
Assist at
[Quotas and limits](https://developers.google.com/gemini-code-assist/resources/quotas).

### If I upgrade to higher limits for LS CLI and Gemini Code Assist by purchasing a Google AI Pro or Ultra subscription, will Gemini start using my data to improve its machine learning models?
...
Search the
[LS CLI Q&A discussions on GitHub](https://github.com/google-gemini/ls-cli/discussions/categories/q-a)
or
[start a new discussion on GitHub](https://github.com/google-gemini/ls-cli/discussions/new?category=q-a)
