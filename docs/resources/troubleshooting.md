...
- **Error:
  ...
  - **Cause:** This error might occur if LS CLI detects the
...
- **Error:
  ...
  - **Cause:** LS CLI does not currently support your location. For a full
...
- **Error: Command not found (when attempting to run LS CLI with
  `gemini`).**
  - **Cause:** LS CLI is not correctly installed or it is not in your
    system's `PATH`.
  - **Solution:** The update depends on how you installed LS CLI:
    - If you installed `gemini` globally, check that your `npm` global binary
      directory is in your `PATH`. You can update LS CLI using the command
    ...
    - If you are running `gemini` from source, ensure you are using the correct
      ...
      To update LS CLI, pull the latest changes from the repository, and
...
- **Error: "Operation not permitted", "Permission denied", or similar.**
  - **Cause:** When sandboxing is enabled, LS CLI may attempt operations
...
- **LS CLI is not running in interactive mode in "CI" environments**
  - **Issue:** LS CLI does not enter interactive mode (no prompt appears) if
  ...
  - **Cause:** The `is-in-ci` package checks for the presence of `CI`,
    ...
    non-interactive, which prevents LS CLI from starting in its interactive
    mode.
...
- **DEBUG mode not working from project .env file**
  - **Issue:** Setting `DEBUG=true` in a project's `.env` file doesn't enable
    debug mode for ls-cli.
  - **Cause:** The `DEBUG` and `DEBUG_MODE` variables are automatically excluded
    from project `.env` files to prevent interference with ls-cli behavior.
...
- **Warning: `npm WARN deprecated node-domexception@1.0.0` or
  ...
  - **Issue:** When installing or updating LS CLI globally via
    `npm install -g @google/ls-cli` or `npm update -g @google/ls-cli`,
  ...
  - **Cause:** These warnings occur because some dependencies (or their
    ...
    versions. Since LS CLI requires Node.js 20 or higher, the platform's
...
LS CLI uses specific exit codes to indicate the reason for termination. This
...
guide_, consider searching LS CLI
[Issue tracker on GitHub](https://github.com/google-gemini/ls-cli/issues).
