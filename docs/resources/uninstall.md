...
`brew install ls-cli`), use the `brew uninstall` command to remove it.
...
`sudo port install ls-cli`), use the `port uninstall` command to remove it.

```bash
sudo port uninstall ls-cli
```
