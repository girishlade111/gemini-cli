...
The `get_internal_docs` tool lets LS CLI access its own technical
documentation to provide more accurate answers about its capabilities and usage.

## Description

This tool is used when LS CLI needs to verify specific details about Gemini
...
The `get_internal_docs` tool is used exclusively by LS CLI. You cannot
invoke this tool manually.

When LS CLI uses this tool, it retrieves the content of the requested
...
LS CLI uses this tool to ensure technical accuracy:

- **Capability discovery:** If LS CLI is unsure how a feature works, it can
  lookup the corresponding documentation.
- **Reference lookup:** LS CLI can verify slash command sub-commands or
  specific setting names.
- **Self-correction:** LS CLI can use the documentation to correct its
  understanding of LS CLI's system logic.
