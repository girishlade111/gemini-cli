...
1.  Link this extension to your local LS CLI installation:

    ```bash
    gemini extensions link packages/cli/src/commands/extensions/examples/policies
    ```

2.  Restart your LS CLI session.
...
For security, LS CLI ignores any `allow` decisions or `yolo` mode
