...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/google-github-actions/run-ls-cli/releases/latest`;
    ...
    debugLogger.debug(
      `Failed to determine latest run-ls-cli release:`,
      error,
    );
    throw new Error(
      `Unable to determine the latest run-ls-cli release on GitHub.`,
    );
  }
};
