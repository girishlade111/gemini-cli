...
export async function start_sandbox(
  ...
  try {
    ...
    // determine full path for ls-cli to distinguish linked vs installed setting
    ...
    // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under ls-cli repo
    //
    // note this can only be done with binary linked from ls-cli repo
    if (process.env['BUILD_SANDBOX']) {
      if (!gcPath.includes('ls-cli/packages/')) {
        throw new FatalSandboxError(
          'Cannot build sandbox using installed gemini binary; ' +
            'run `npm link ./packages/cli` under ls-cli repo to switch to linked binary.',
        );
      } else {
    ...
    if (!(await ensureSandboxImageIsPresent(command, image, cliConfig))) {
      const remedy =
        image === LOCAL_DEV_SANDBOX_IMAGE_NAME
          ? 'Try running `npm run build:all` or `npm run build:sandbox` under the ls-cli repo to build it locally, or check the image name and your network connection.'
          : 'Please check the image name, your network connection, or notify ls-cli-dev@google.com if the issue persists.';
      throw new FatalSandboxError(
