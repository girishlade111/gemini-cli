...
describe('Storage - System Paths', () => {
  ...
  it('getSystemSettingsPath returns correct path based on platform (default)', () => {
    ...
    if (platform === 'darwin') {
      ...
      expect(result).toBe('C:\\ProgramData\\ls-cli\\settings.json');
    } else {
      expect(result).toBe('/etc/ls-cli/settings.json');
    }
  });
  ...
  it('getSystemPoliciesDir returns correct path based on platform and ignores env var', () => {
    ...
    if (platform === 'darwin') {
      ...
      expect(result).toBe('C:\\ProgramData\\ls-cli\\policies');
    } else {
      expect(result).toBe('/etc/ls-cli/policies');
    }
  });
});
