...
function getSeccompBpfPath(): string {
  ...
  const tempDir = fs.mkdtempSync(join(os.tmpdir(), 'ls-cli-seccomp-'));
...
export class LinuxSandboxManager implements SandboxManager {
  ...
  private getMaskFilePath(): string {
    ...
    const tempDir = fs.mkdtempSync(join(os.tmpdir(), 'ls-cli-mask-file-'));
  ...
  private writeArgsToTempFile(args: string[]): string {
    const tempFile = join(
      os.tmpdir(),
      `ls-cli-bwrap-args-${Date.now()}-${Math.random().toString(36).slice(2)}.args`,
    );
