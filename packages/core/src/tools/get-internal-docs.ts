...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find LS CLI documentation directory.');
}
  ...
  async execute({ abortSignal: _signal }: ExecuteOptions): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available LS CLI documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to LS CLI's internal documentation.
