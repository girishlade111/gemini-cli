...
function checkRelease() {
  try {
    ...
    const bucketUri = 'gs://ls-cli-vscode-extension/release/1p/signed';
    ...
    const vsixFiles = files.filter((file) =>
      /signed-ls-cli-vscode-ide-companion-\d+\.\d+\.\d+-[a-f0-9]{7}\.vsix$/.test(
        file,
      ),
    );
    ...
    const match =
      /signed-ls-cli-vscode-ide-companion-(\d+\.\d+\.\d+)-([a-f0-9]{7})\.vsix$/.exec(
        fileName,
      );
