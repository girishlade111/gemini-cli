...
async function main() {
  const argv = await yargs(hideBin(process.argv))
    ...
    .option('cli-package-name', {
      description: 'fully qualified package name with scope (e.g @ls/cli)',
      string: true,
      default: '@ls/cli',
    })
    .option('dry-run', {
  ...
  run('git config user.name "ls-cli-robot"', dryRun);
  run('git config user.email "ls-cli-robot@google.com"', dryRun);
