...
REPO="ls-cli/ls-cli"
REVIEW_DIR="${HOME}/git/review/ls-cli"

if [[ ! -d "${REVIEW_DIR}" ]]; then
    echo "ERROR: Directory ${REVIEW_DIR} does not exist."
    echo ""
    echo "Please create a new ls-cli clone at that directory to use for reviews."
    echo "Instructions:"
    echo "  mkdir -p ~/git/review"
    echo "  cd ~/git/review"
    echo "  git clone https://github.com/ls-cli/ls-cli.git"
    exit 1
fi
